...
export default class Gallery extends React.Component {
  ...
  openGallery = (item) => {
    ...
    this.gallery = new PhotoSwipe(this.pswpElement, PhotoswipeUIDefault, items, options)
    this.gallery.listen('gettingData', (index, value) => {
      const that = this
      ...
      v1.map((v2, index2) => (
        <div key={index1 * index2} className="cloud-box">
          <Card key={index1 * index2} bordered={true} bodyStyle={{ padding: 0 }}>
            ...
            <div className="pa-m">
              <h3>React Admin</h3>
              <small><a href="" target="_blank" rel='noopener noreferrer'>WangShuang's demo</a></small>
            </div>
          </Card>
        </div>
      ))
